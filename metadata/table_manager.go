...
func (tblMgr *TableManager) createTable(tableName string, schema *record.Schema, txn *tx.Transaction) error {
	...
	ts, err := scan_types.NewTableScan(txn, "tblcat", tblMgr.tableCatalogLayout)
	if err != nil {
		return err
	}
	...
	if ts, err = scan_types.NewTableScan(txn, "fldcat", tblMgr.fldCatalogLayout); err != nil {
		return err
	}
...
func (tblMgr *TableManager) getLayout(tblname string, txn *tx.Transaction) (*record.Layout, error) {
	...
	ts, err = scan_types.NewTableScan(txn, "fldcat", tblMgr.fldCatalogLayout)
	if err != nil {
		return nil, err
	}
